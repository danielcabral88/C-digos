...
Graph cria(int qtd_vertex){
    ...
    G->inicio = (int*) malloc(sizeof(int)*(qtd_vertex + 1)); // aloca memória para o ponteiro inicio
    G->fim = (int*) malloc(sizeof(int)*(qtd_vertex + 1)); // aloca memória para o ponteiro fim
    G->cor = (int*) malloc(sizeof(int)*(qtd_vertex + 1)); // aloca memória para o ponteiro cor
...
void busca_em_largura(Graph g, int k){
    ...
    while (q.size() > 0){
		...
		for (aux2 = 0 ; aux2 < aux3 ; aux2++){ // com a quantidade de adjacentes verifica se já foi visitado
...
int dfs_visit(Graph g, int k, int cont)
{
    int aux;
	g->cor[k] = cinza;
    g->inicio[k] = cont++; // inicia do elemento posterior ao passado como parâmetro
    for (aux = 0; aux < g->qtd_vertex ; aux++){
